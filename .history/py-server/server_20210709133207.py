...
for x in col_name.distinct("location"):
    ...
    for y in col_name.find():
        ...
        if x == y["location"]:
            if y["cond"] == "cancer":
                x = other_col.find_one({"location": x})
                og_count = x["cancer_count"]

                other_col.find_and_modify( query={"location": x}, update={"$set": {"cancer_count": og_count}})
                print("updated_canc")

            if y["cond"] == "typhoid":
                y = other_col.find_one({"location" : x})
                print(y)
                og_count = int(y["typhoid_count"])
                og_count = og_count+1
                other_col.find_and_modify( query={"location": x}, update={"$set": {"typhoid_count": og_count}})
                print("updated_typ")
